...
<script>
	export default {
		...
		methods: {
			...
			deleteItem(row) {
				console.log(row);
				this.$axios.delete("/importitem/" + row.id).then(res => {
			...
			updateItem() {
				...
				this.$axios.post("/importitem/" + this.form.id, {
					id: this.form.id,
					name: this.form.name,
			...
			getData(page, size) {
				...
				this.$axios.get("http://localhost:8081/importitem/importitem/" + page + '/' + size).then(res => {
					if (res.data.code == '200') {
						this.$message({
							message: res.data.msg,
							type: 'success',
