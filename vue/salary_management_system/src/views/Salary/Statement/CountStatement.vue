<template>
	<div>
		<el-form ref="form" :model="form" label-width="150px" style="margin-left: 300px;">
			...
			<el-button type="primary" @click="count" style="margin-left: 200px; margin-top: 50px;">立即计算</el-button>
			</el-form-item>
		</el-form>
	</div>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			count() {
				this.$axios.get("/statement/count/" + this.department + "/" + this.month).then(res => {
					if (res.data.code == '200') {
						this.$message({
							type: 'success',
