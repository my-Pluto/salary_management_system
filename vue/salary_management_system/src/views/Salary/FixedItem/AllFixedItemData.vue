 ...
 -->
		...
		<el-dialog title="查询" :visible.sync="dialogTableVisible">
			<el-form label-width="200px">
				...
				<el-form-item label="请选择固定项目" style="margin-top: 3.125rem;">
					<el-select v-model="itemname" placeholder="请选择固定项目">
						<el-option v-for="item in itemnames" :key="item.name" :label="item.name" :value="item.name" />
					</el-select>
				</el-form-item>
				<el-form-item label="请输入金额" style="margin-top: 3.125rem; width: 420px;">
					<el-input v-model.number="lowsalary" placeholder="最低金额"></el-input>
					<el-input v-model.number="highsalary" placeholder="最高金额"></el-input>
				</el-form-item>
			</el-form>
...
<script>
	export default {
		...
		methods: {
			...
			// 		this.$axios.post("/fixeditemdata//fixeditemdata/" + row.id, {
			// 			item: value
			// 		}).then(res => {
			// 			if (res.data.code == '200') {
			// 				this.$message({
			...
			getData(page, size) {
				...
				this.$axios.post("/fixeditemdata/fixeditemdata/" + page + "/" + size, {
